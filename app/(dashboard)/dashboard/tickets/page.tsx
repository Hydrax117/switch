...
import { Ticket, Calendar, MapPin, QrCode, Users, ArrowRight, CheckCircle2, Clock, XCircle } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-[14px]">
          ...
          {groupOrders.length > 0 && ` · ${groupOrders.length} group booking${groupOrders.length !== 1 ? 's' : ''}`}
        </p>
      </div>
      ...
      <div className="border-border flex gap-1 border-b">
        <TabLink href="/dashboard/tickets?tab=tickets" active={activeTab === 'tickets'}>
          ...
          {tickets.length > 0 && (
            <span className={cn(
              ...
              activeTab === 'tickets' ? 'bg-brand-600 text-white' : 'bg-muted text-muted-foreground'
            )}>
              {tickets.length}
            </span>
          )}
        ...
        <TabLink href="/dashboard/tickets?tab=groups" active={activeTab === 'groups'}>
          ...
          {groupOrders.length > 0 && (
            <span className={cn(
              'ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] font-bold',
              activeTab === 'groups' ? 'bg-brand-600 text-white' : 'bg-muted text-muted-foreground'
            )}>
              {groupOrders.length}
            </span>
          )}
...
const GROUP_STATUS_CONFIG = {
  ...
  COMPLETE: { label: 'Complete', icon: CheckCircle2, className: 'bg-emerald-500/10 text-emerald-400' },
    ...
    >
      <div className="flex items-center gap-4 p-4">
        ...
        <div className="shrink-0 text-right">
          <div className={cn('inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-[11px] font-semibold', config.className)}>
