...
export async function sendTicketConfirmationEmail(params: {
  ...
  // Generate PDF attachment (all tickets, one page each)
  ...
  // Generate inline QR data URLs (up to 6 shown in the email body)
  ...
  // Build one table row per visible ticket
  const ticketRowsHtml = ticketsToShow
    .map((t, i) => {
      const typeLabel = t.ticketTypeName + (t.seatLabel ? ` · Seat ${t.seatLabel}` : '')
      return (
        '<tr>' +
        '<td style="padding:16px 0;border-bottom:1px solid #27272a;">' +
        '<table width="100%" cellpadding="0" cellspacing="0"><tr>' +
        '<td style="vertical-align:top;padding-right:16px;">' +
        '<p style="margin:0 0 2px;font-size:14px;font-weight:600;color:#fafafa;">' + typeLabel + '</p>' +
        '<p style="margin:0;font-size:11px;font-family:monospace;color:#71717a;letter-spacing:0.05em;">' + t.ticketNumber + '</p>' +
        '</td>' +
        '<td style="vertical-align:top;text-align:right;width:84px;">' +
        '<img src="' + qrDataUrls[i] + '" width="76" height="76" alt="QR ' + t.ticketNumber + '" style="border-radius:6px;display:block;margin-left:auto;" />' +
        '</td>' +
        '</tr></table>' +
        '</td>' +
        '</tr>'
  ...
  const extraNote = extraCount > 0
    ? '<p style="margin:12px 0 0;font-size:12px;color:#71717a;">+ ' + extraCount + ' more ticket' + (extraCount !== 1 ? 's' : '') + ' — all included in the PDF attachment</p>'
    : ''

  const pluralTickets = params.ticketCount !== 1 ? 's' : ''
  ...
  const html =
    ...
    // Event block
    '<tr><td style="background:#18181b;border-radius:12px;padding:20px;">' +
    ...
    // PDF notice
    '<tr><td style="background:#1c1917;border:1px solid #292524;border-radius:12px;padding:14px 20px;">' +
    '<p style="margin:0;font-size:13px;color:#d6d3d1;">&#128206; <strong style="color:#fafafa;">Your ticket' + pluralTickets + ' are attached</strong> as a PDF — save it to your phone or print it to get in offline.</p>' +
    '</td></tr>' +
    ...
    // Tickets
    '<tr><td style="background:#18181b;border-radius:12px;padding:20px;">' +
    '<p style="margin:0 0 4px;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#71717a;">Your ticket' + pluralTickets + ' (' + params.ticketCount + ')</p>' +
    '<table width="100%" cellpadding="0" cellspacing="0">' + ticketRowsHtml + '</table>' +
    extraNote +
    '</td></tr>' +
    ...
    // CTA
    '<tr><td align="center">' +
    '<a href="' + appUrl + '/dashboard/tickets" style="display:inline-block;background:#6366f1;color:#fff;font-weight:600;font-size:14px;padding:13px 28px;border-radius:10px;text-decoration:none;">View all my tickets &#8594;</a>' +
    '</td></tr>' +
    ...
    // Footer
    '<tr><td>' +
    '<p style="margin:0;font-size:12px;color:#52525b;text-align:center;">Booking ref: <span style="font-family:monospace;">' + params.reservationId + '</span></p>' +
    '<p style="margin:6px 0 0;font-size:11px;color:#3f3f46;text-align:center;">Present the PDF or QR code at the entrance.</p>' +
    '</td></tr>' +
  ...
  const { error } = await resend.emails.send({
    ...
    attachments: [
      {
        filename: `SWITCH-tickets-${params.reservationId}.pdf`,
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:12px;">
        ...
        <a href="${joinUrl}" style="display:block;text-align:center;background:#6366f1;color:#fff;font-weight:600;font-size:15px;padding:14px 24px;border-radius:10px;text-decoration:none;margin-bottom:24px;">
          Claim my spot →
        </a>
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:12px;">
        ...
        <a href="${appUrl}/dashboard/tickets" style="display:block;text-align:center;background:#6366f1;color:#fff;font-weight:600;font-size:15px;padding:14px 24px;border-radius:10px;text-decoration:none;">
          View my tickets →
        </a>
      </div>
    `,
